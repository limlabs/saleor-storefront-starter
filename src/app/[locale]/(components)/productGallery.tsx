	...
	return (
		<>
			...
			<div className="flex justify-between">
				{hasPreviousPage? (<Link href={`/${locale}/c/all?before=${startCursor}`}>
                	<span className='text-base-content/50 flex flex-row'>
                    	<ChevronLeft/> Prev
                    </span>
				</Link>): (<span className='text-base-content/50 flex flex-row'>
                    	<ChevronLeft/> Prev
                    </span>)}
				
				{hasNextPage?(<Link href={`/${locale}/c/all?after=${endCursor}`}>
					<span className='text-base-content/50 flex flex-row'>
						Next <ChevronRight/>
					</span>
				</Link>):(<span className='text-base-content/50 flex flex-row'>
